...
void ProcessCommand(string jsonCommand)
{
   ...
   else if(action == "get_symbol_info")
   {
      ...
      Print("SYMBOL INFO: symbol=", symbol);
      WriteSymbolInfo(symbol);
   }
...
//+------------------------------------------------------------------+
//| Write symbol info for risk calculator                            |
//+------------------------------------------------------------------+
void WriteSymbolInfo(string symbol)
{
   ...
   json += "\"pipSize\":" + DoubleToString(pipSize, 8) + ",";
   ...
   json += "\"contractSize\":" + DoubleToString(contractSize, 0) + ",";
   json += "\"bid\":" + DoubleToString(bid, digits) + ",";
   json += "\"ask\":" + DoubleToString(ask, digits) + ",";
   ...
   if(handle != INVALID_HANDLE)
   {
      FileWriteString(handle, json);
      FileClose(handle);
      Print("Symbol info written: ", symbol, " pipValue=", pipValue);
   }
}
